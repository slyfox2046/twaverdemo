...
;!function(a) {
    var b = {
        ...
        isv: !1,
        onlytime: false,//是否只显示时间
        onlyMonth: true //是否只显示月
    }
      , c = {}
      , d = document
      , e = "createElement"
    ...
    c.check = function() {
        ...
        if(c.options.onlytime||c.options.onlyMonth){
            f.value = c.elem[h.elemv];
            return true;
        }
        if ("" !== d.replace(/\s/g, "")) {
            if (!b.test(d))
    ...
    c.initDate = function() {
        ...
        var l = c.elem[h.elemv].match(/\d+/g) || [];
        e.length < 3 && (e = c.options.start.match(/\d+/g) || [],
        e.length < 3 && (e = [d.getFullYear(), d.getMonth() + 1, d.getDate()]));
        if(c.options.onlyMonth && l.length == 2){
            e = c.elem[h.elemv].match(/\d+/g) || [];
            e[2] = 1;
        };
        if(c.options.onlytime && l.length == 3){
            e[3] =l[0];
            e[4] =l[1];
            e[5] =l[2];
        };
    ...
    c.iswrite = function() {
        ...
        $(".laydate_body .laydate_table,.laydate_body .laydate_top").show();
        if(c.options.onlytime){
            $(".laydate_body .laydate_table,.laydate_body .laydate_top, #laydate_today").hide();
        };
        if(c.options.onlyMonth){
            $(".laydate_body .laydate_table,#laydate_today,.laydate_body .laydate_bottom #laydate_hms").hide();
        };
    }
    ,
    ...
    c.creation = function(a, b) {
        ...
        if(c.options.onlytime) {
          f =   [e[0].value +":" +e[1].value +":"+  e[2].value];
        };
        if(c.options.onlyMonth) {
          f =   f.substr(0,7);
            var month = f.substr(0,4);
            c.viewYears(month);
        };
        c.elem[h.elemv] = f,
    ...
    c.events = function() {
        ...
        c.on(h.ok, "click", function() {
            if(c.options.onlytime||c.options.onlyMonth){
                c.creation([c.ymd[0], c.ymd[1] + 1, c.ymd[2]]);
                return true;
            }
            ...
            c.each(e, function(b, e) {
                ...
                if(c.options.onlytime){
                     c.on(e, "click", function() {
                        a.value = c.digit(0 | this.innerHTML);
                        c.hasClass(this, h[1]) ;
                    })
                };
            }),
        ...
        c.on(d, "mouseup", function() {
            ...
            if(c.options.onlytime||c.options.onlyMonth){
                a && "none" !== a.style.display && (c.close());
            }
